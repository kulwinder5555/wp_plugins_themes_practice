 ...
 * @since Boklium 1.0.0
 */
class Boklium_EDD_Updater_Admin {
	...
	function updater() {
		...
		if ( !class_exists( 'Boklium_EDD_Updater' ) ) {
			// Load our custom theme updater
			include( dirname( __FILE__ ) . '/theme-updater-class.php' );
		}

		new Boklium_EDD_Updater(
			array(
	 ...
	 */
	 function get_api_response( $api_params ) {
		...
		if ( is_wp_error( $response ) ) {
			wp_die( $response->get_error_message(), esc_html__( 'Error','booklium' ) . $response->get_error_code() );
		}
